...
.directive('json', function($compile, state) {
  return {
    restrict: 'E',
    scope: {
      child: '=',
    ...
    link: function(scope, element, attributes) {
        ...
        //custom jspath stuffs
        var parPath = scope.$parent.jspath || "";
        scope.makeObjectPath = function(key) {
            var keyStr;
            if (/^[A-Za-z_]*$/.test(key)) {
                //only normal chars
                keyStr = '.'+key;
            ...
            return parPath + keyStr;
        };
        scope.makeArrayPath = function(index) {
            return [parPath, "[", index, "]"].join("");
        };
        scope.setResults = function(path) {
            state.getState().then(function(state) {
                state.setResults(path);
            });
        };
        ...
        var switchTemplate = 
            '<span ng-switch on="getType(val)" >'
                + '<json ng-switch-when="Object" child="val" type="object" default-collapsed="defaultCollapsed" jspath="jspath" showsetresults="showsetresults"></json>'
                + '<json ng-switch-when="Array" child="val" type="array" default-collapsed="defaultCollapsed" jspath="jspath" showsetresults="showsetresults"></json>'
        ...
        var setResultsTemplate = scope.showsetresults ?
            ' <a ng-click="setResults(jspath)" ng-show="jspath" class="setResults">set as results</a>' : "";
    
        // start template
        if (scope.type == "object"){
            var template = '<i ng-click="toggleCollapse()" class="glyphicon" ng-class="chevron"></i>'
            + '<span class="jsonItemDesc">'+objectName+'</span>' + setResultsTemplate
            + '<div class="jsonContents" ng-hide="collapsed">'
                // repeat
                + '<span class="block" ng-repeat="(key, val) in child">'
                    // object key
                    + '<span class="jsonObjectKey" ng-init="jspath=makeObjectPath(key)">'
            ...
            + 'ng-class="chevron"></i>'
            + '<span class="jsonItemDesc">'+arrayName+'</span>' + setResultsTemplate
            + '<div class="jsonContents" ng-hide="collapsed">'
                + '<ol class="arrayOl" ui-sortable="sortableOptions" ng-model="child">'
                    // repeat
                    + '<li class="arrayItem" ng-repeat="val in child" ng-init="jspath=makeArrayPath($index)">'
                        // delete button
